            ...
            if (Int32.TryParse(Console.ReadLine(),out job.J_id))
            {
                ...
                if (Int32.TryParse(Console.ReadLine(), out job.J_size))
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (job.J_size < memlist[i].M_size)
                            {
                                job.J_start = memlist[i].M_start;//进行作业起始地址赋值；
                                memlist[i].M_start = job.J_start + job.J_size;//空闲起始重划分；
                                memlist[i].M_size -= job.J_size;//空闲地址长度减小；
        ...
        public static void Return(int job_id)//回收
        {
            foreach (Job jb in joblist)
            {
                if (jb.J_id == job_id)
                {
                    ...
                    memlist.Sort(delegate(Memory x, Memory y)
                    {
